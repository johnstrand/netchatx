...
public class TcpTlsTransportTests
{
    ...
    [Fact]
    public async Task UpgradeToTlsAsync_UntrustedCertificate_WhenAllowed_Succeeds()
    {
        // Generate a self-signed certificate (untrusted by standard validation)
        using var rsa = RSA.Create(2048);
        var req = new CertificateRequest("CN=localhost", rsa, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
        ...
        try
        {
            await using var transport = new TcpTlsTransport(allowUntrustedCertificates: true);
            await transport.ConnectAsync("127.0.0.1", port);

            // With allowUntrustedCertificates: true, upgrade must succeed
            await transport.UpgradeToTlsAsync("localhost");
