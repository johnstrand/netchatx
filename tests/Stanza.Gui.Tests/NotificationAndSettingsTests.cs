...
public class NotificationAndSettingsTests : IDisposable
{
    ...
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"stanza_autostart_test_{Guid.NewGuid():N}.desktop");
        var tempExe = "/usr/bin/stanza";
        ...
        try
        {
            ...
            Assert.Contains("/usr/bin/stanza", content);
