...
public class EmoticonReplacementTests
{
    ...
    public void EmoticonReplacer_ReplacesStandardEmoticons()
    {
        ...
        var (result3, replaced3) = EmoticonReplacer.ReplaceEmoticons("I <3 Stanza!", mappings);
        Assert.True(replaced3);
        Assert.Equal("I ❤️ Stanza!", result3);
    }
    ...
    {
        var dbPath = Path.Combine(Path.GetTempPath(), $"stanza_emoticon_test_{Guid.NewGuid():N}.db");
        try
        {
    ...
    {
        var dbPath = Path.Combine(Path.GetTempPath(), $"stanza_settings_emoticon_{Guid.NewGuid():N}.db");
        try
        {
            var dbContext = new DatabaseContext(dbPath);
            var repo = new SettingsRepository(dbContext);
