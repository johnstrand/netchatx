using System.Text;
using NetChatx.Core.Sasl;
using Xunit;

namespace NetChatx.Core.Tests;

public class SaslTests
{
    [Fact]
    public void PlainSasl_GeneratesCorrectNullSeparatedBase64()
    {
        var sasl = new PlainSaslMechanism();
        string? response = sasl.CreateInitialResponse("alice", "secret");

        Assert.NotNull(response);
        byte[] bytes = Convert.FromBase64String(response);

        // Expected: \0alice\0secret
        byte[] expected = Encoding.UTF8.GetBytes("\0alice\0secret");
        Assert.Equal(expected, bytes);
    }

    [Fact]
    public void ScramSha256_ChallengeAndProofRoundtrip_Succeeds()
    {
        var client = new ScramSaslMechanism(isSha256: true);
        string? clientFirstB64 = client.CreateInitialResponse("user", "pencil");
        Assert.NotNull(clientFirstB64);

        string clientFirst = Encoding.UTF8.GetString(Convert.FromBase64String(clientFirstB64));
        Assert.StartsWith("n,,n=user,r=", clientFirst);
        string clientNonce = clientFirst.Substring("n,,n=user,r=".Length);

        // Simulate server challenge
        string serverNonce = clientNonce + "servernonce123";
        string saltB64 = Convert.ToBase64String(Encoding.UTF8.GetBytes("random_salt"));
        string serverFirst = $"r={serverNonce},s={saltB64},i=4096";
        string serverFirstB64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(serverFirst));

        string? clientFinalB64 = client.HandleChallenge(serverFirstB64, "pencil");
        Assert.NotNull(clientFinalB64);

        string clientFinal = Encoding.UTF8.GetString(Convert.FromBase64String(clientFinalB64));
        Assert.StartsWith($"c=biws,r={serverNonce},p=", clientFinal);
    }
}
