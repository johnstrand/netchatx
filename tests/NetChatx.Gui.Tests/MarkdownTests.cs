...
public class MarkdownTests
{
    ...
    {
        var doc = MarkdownParser.Parse("Visit [NetChatx Repo](https://github.com/johnstrand/netchatx) today!");
        var p = Assert.IsType<MarkdownParagraph>(doc.Blocks[0]);
        Assert.Equal(3, p.Inlines.Count);
        var link = Assert.IsType<MarkdownLink>(p.Inlines[1]);
        Assert.Equal("NetChatx Repo", link.Text);
        Assert.Equal("https://github.com/johnstrand/netchatx", link.Url);
    }
    ...
    public void Xep0393_MessageStyling_DiscoveryAndUnstyledHelpers()
    {
        ...
        var msg = new NetChatx.Core.Stanzas.MessageStanza(to: NetChatx.Core.Jid.Parse("peer@example.com"), body: "> _ <");
