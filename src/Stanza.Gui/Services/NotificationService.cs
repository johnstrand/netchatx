...
public sealed class NotificationService : INotificationService
{
    ...
    private static void ShowWindowsPowerShellToast(string title, string message)
    {
        Task.Run(() =>
        {
            var tempFile = Path.Combine(Path.GetTempPath(), $"stanza_toast_{Guid.NewGuid():N}.ps1");
            try
            {
...
try {{
    ...
    try {{
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Stanza').Show($toast)
    }} catch {{
...
";
    ...
    private static void ShowMacNotification(string title, string message)
    {
        ...
        try
        {
            ...
            var script = $"display notification \"{safeMessage}\" with title \"Stanza\" subtitle \"{safeTitle}\" sound name \"default\"";
